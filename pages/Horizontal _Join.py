...
uploaded_file = st.file_uploader(label="Choose a file", accept_multiple_files=True,type=['tsv', 'csv', 'json', 'xlsx'])
...
if uploaded_file is not None:
    ...
    if st.button('Get Transpose'):
        try:
            API_url = "http://127.0.0.1:8000/autoworkflow/horizontal-join"
            files=[]
            for file in uploaded_file:
                files.append(("upload_files", (file.name, file.getvalue(), "multipart/form-data")))
            
            with requests.post(API_url, files=files, stream=True) as r:
                if r.status_code == 200:
                    ...
                    st.text('Output ready! Click below button to download.')
