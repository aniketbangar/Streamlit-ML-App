...
# API endpoint URL
FILTER_URL = 'http://localhost:8000/autoworkflow/melt-dataframe'
...
if uploaded_file is not None:
    ...
    id_columns = st.text_input("ID Coulumns (comma-separated)", value='').split(',')
    value_columns = st.text_input("Value Columns (comma-separated)", value='').split(',')
    data = {
        'id_columns': [x.strip() for x in id_columns if x.strip() != ''],
        'value_columns': [x.strip() for x in value_columns if x.strip() != ''],
    }
